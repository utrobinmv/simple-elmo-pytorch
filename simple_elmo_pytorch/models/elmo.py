...
from simple_elmo_pytorch.models.token_embedders import Embedding  # type: ignore


#from allennlp.common import FromParams
#from allennlp.common.checks import ConfigurationError
from simple_elmo_pytorch.file_utils import cached_path
from simple_elmo_pytorch.util import lazy_groups_of
from simple_elmo_pytorch.data.instance import Instance
from simple_elmo_pytorch.data.tokenizers import Token
from simple_elmo_pytorch.data.vocabulary import Vocabulary
from simple_elmo_pytorch.data.batch import Batch
from simple_elmo_pytorch.data.fields import TextField
from simple_elmo_pytorch.data.elmo_indexer import (
    ELMoCharacterMapper,
    ELMoTokenCharactersIndexer,
)


from simple_elmo_pytorch.models.elmo_lstm import ElmoLstm
from simple_elmo_pytorch.models.scalar_mix import ScalarMix


from simple_elmo_pytorch.models.highway import Highway

from simple_elmo_pytorch.nn.util import (
...
class Elmo(torch.nn.Module): #, FromParams
    ...
    module : `torch.nn.Module`, optional, (default = `None`).
        ...
        (see `_ElmoBilm (ElmoBiLmSentence)` for an example).  Note that `requires_grad` is also
    ...
    def __init__(
        ...
        if module is not None:
            ...
            self._elmo_lstm = ElmoBiLmSentence(  # type: ignore
                options_file,
                weight_file,
    ...
    def reinit(self):
        self._scalar_mixes: Any = []
        for k in range(self.num_output_representations):
            scalar_mix = ScalarMix(
                self._elmo_lstm.num_layers,  # type: ignore
                do_layer_norm=self.do_layer_norm,
                initial_scalar_parameters=self.scalar_mix_parameters,
                trainable=self.scalar_mix_parameters is None,
            )
            self.add_module("scalar_mix_{}".format(k), scalar_mix)
            self._scalar_mixes.append(scalar_mix)
    ...
    def forward(
        ...
        ret = bilm_output
        ret['elmo_representations']  = elmo_representations
        ret['mask']  = mask
...
class ElmoBiLmSentence(torch.nn.Module):
    """
    ...
    vocab_to_cache : `List[str]`, optional, (default = `None`).
        A list of words to pre-compute and cache character convolutions
        for. If you use this option, _ElmoBiLm (ElmoBiLmSentence) expects that you pass word
        indices of shape (batch_size, timesteps) to forward, instead
        of character indices. If you use this option and pass a word which
        wasn't pre-cached, this will break.
    """
